 ...
 *    在实际操作的时候，一般只统计单侧，下面的算法就只统计了穿过右侧的次数
 *    偶数：在多边形外；     奇数：在多边形内
 *
 ...
 *   然后再以面积为权值,加权求取最终的质心
 *   参考:http://www.cnblogs.com/jbelial/archive/2011/08/08/2131165.html
 * ]
 * @return
 */
...
bool Polygon::convertToInequalityConstraints(Eigen::MatrixXd& A, Eigen::VectorXd& b) const
{
    ...
    // 建立一个1行n列的矩阵，c为所有点x，y的均值，含有两个值的列向量
                                   ...
                                   const int nVertices)
{
    ...
    {
        // 2.1 计算旋转角度 theta = π/2 + j/(0.5*N-1) * π
